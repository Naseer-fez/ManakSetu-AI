...
export const UnifiedItemsList: React.FC<UnifiedItemsListProps> = ({ items }) => {
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-3">
        {items.map(item => {
          const cleanTitle = item.product_title.replace(/^\*+\s*|\*+/g, "").trim();
          return (
            <div key={item.item_id} className="p-4 rounded-xl bg-white dark:bg-[#111927] border border-gov-border dark:border-slate-800 space-y-2.5 transition-all shadow-2xs hover:border-gray-300 dark:hover:border-slate-700">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div className="flex items-center gap-2">
                  <span className="px-2 py-0.5 rounded bg-blue-50 dark:bg-blue-950/60 text-gov-blue dark:text-blue-300 text-[10px] font-bold">Item #{item.item_id}</span>
                  <h4 className="text-xs font-bold text-gov-navy dark:text-white tracking-tight">{cleanTitle}</h4>
                </div>
              ...
              <div className="text-xs text-gov-text dark:text-gray-200 leading-relaxed bg-gov-offwhite dark:bg-[#0c1421] p-3 rounded-lg border border-gov-border dark:border-slate-800/80">
                <MarkdownRenderer content={item.spec_summary} className="text-xs leading-relaxed" />
              </div>
