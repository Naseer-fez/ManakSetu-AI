          ...
          {findings.map((item, idx) => (
            <div key={idx} className="p-3.5 rounded-lg bg-white dark:bg-[#111927] border border-gov-border dark:border-slate-800 space-y-2 transition-all shadow-2xs hover:border-gray-300 dark:hover:border-slate-700">
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs font-bold text-gov-navy dark:text-white truncate">{item.category}</span>
              ...
              <p className="text-xs text-gov-text dark:text-gray-300 leading-relaxed">{item.message}</p>
              <div className="flex justify-end pt-1">
                <button
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-blue-50 dark:bg-blue-950/40 hover:bg-gov-blue dark:hover:bg-blue-700 text-gov-blue dark:text-blue-400 hover:text-white dark:hover:text-white text-[11px] font-medium transition-colors border border-blue-200 dark:border-blue-900/50"
